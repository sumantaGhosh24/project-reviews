...
type ReplyFormType = z.infer<typeof createReplySchema>;
...
export function ReplyForm({commentId, releaseId}: ReplyFormProps) {
  ...
  const form = useForm<ReplyFormType>({
  ...
  const onSubmit = async (values: ReplyFormType) => {
    if (files) {
    ...
    <div className="rounded-lg border bg-background p-2">
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <FieldGroup className="flex flex-row items-center gap-2">
          ...
          >
            <LoadingSwap isLoading={createReply.isPending || isUploading}>
              <SendIcon />
            </LoadingSwap>
          </Button>
        </FieldGroup>
